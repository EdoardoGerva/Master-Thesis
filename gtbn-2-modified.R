#Epi-GTBN gtbn2 code modified by Edoardo Gervasoni, e.gervasoni4@campus.unimib.it
...
gtbn.2 = function(x, start, whitelist, blacklist, score, extra.args,
    ...
    if(debug){
        cat("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ GTBN 2 NODES DEBUG MODE PROVAEDO ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
		...
		print(blacklist)
		cat("============ Version Modified by Edoardo Gervasoni - contact at e.gervasoni4@campus.unimib.it ============\n")
	}
	...
	#cat("\n======== ",extra.args)
	cat("\n======== ",reference.score)
	cat("\n======== ",score.equivalence && optimized)
	cat("\n======== ",score.decomposability)
	cat("\n======== ",(if (optimized) updated else seq(length(nodes)) - 1L))
	cat("\n======== ",cache_ref)
	cat("\n======== ",blmat)
	cat("\n======== ",debug)
    ...
    #cat("\n======== Guardiamo a x_t con classe in alto ========\n")
    #cat(x_t)
    cat("\n======== Guardiamo a x_t classe ========\n")
    print(x_t[1,])
    
    ############################--- From here the code is modified ---############################
    ...
    #         }
    # cat("\n======== Guardiamo a relation ========\n")
    # print(data.matrix(relation))
    # relation
    # time2end <- Sys.time()
    ...
    # Sort by value
    # o <- order(relation[, 3], decreasing = T)      #order restituisce le righe corrispondenti.
    # cat("\n======== Guardiamo a o ========\n")
    # print(o)
    # relation_order_all <- relation[o, ]
    # cat("\n======== Guardiamo a relation_order_all ========\n")
    # print(relation_order_all)     #per dataframe usare print
    ...
    # Non-boolean Mutual Information
    
    df <- x
    class <- df[,ncol(df)]
    
    MI <- data.frame(SNP1 = character(),          # Specify empty vectors in data.frame
                     SNP2 = character(),
                     MI = double(),
    ...
    for(i in c(1:(ncol(df)-1))){
      cat(i)
      for(y in c(1:(ncol(df)-1))){
        if(i<y){
          snp1 <- df[,i]
          snp2 <- df[,y]
          value <- mi(class,snp1,snp2)
          #cat('\n   ',l,'  ',colnames(d)[i],'   ',colnames(d)[y],'  ',value)
          
          MI[l_MI, ] <- list(colnames(df)[i],colnames(df)[y],value)
          l_MI<-l_MI+1
        }
      }
    }
    
    ord <- order(MI[, 3], decreasing = T)      #order gives the corresponding rows.
    top <- MI[ord, ]
    relation_order_all_noValCol <- (top[, 1:2])
    # Take the largest 100
    relation_order_all_noValCol_core <- relation_order_all_noValCol[1:100,]
    relation_order_all_noValCol_core
