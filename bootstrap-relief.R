...
#bootstrap procedure for ReliefF data

#read the data
data_relieff  <- read.csv("ReliefF_output_top400_corrected.csv")
#remove missing genotypes
row <- apply(data_relieff,1,function(row) all(row !=-1))
data_relieff <- data_relieff[row,]
...
for(i in c(1:N)){
  cat('\n',i)
  #sampling random rows mantaining class label ratios with replacement -> perturbation
  data_sample <- data_relieff %>% group_by(Y) %>% sample_n(5000, replace =T)
...
score
write.csv(score,file='score_relieff_top400_2.csv')
...
write.csv(arch_scores,file='Undirected_arch_scores_relieff_top400_2.csv')
